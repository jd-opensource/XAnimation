...
- (void)displayWithFrame:(NSNumber *)frame forceUpdate:(BOOL)forceUpdate {
  ...
  _maskLayer.currentFrame = newFrame;
    ...
    //@wxy XAnimation add begin
    NSDictionary *transform3dDic = nil;
    if (_transformInterpolator && [_transformInterpolator hasUpdateForFrame:newFrame]) {
         transform3dDic= [_transformInterpolator transformDicForFrame:newFrame];
    }
  [XABindLottieMiddleWare didUpdateLayer:self frame:newFrame opacity:self.opacity transform3DInfo:transform3dDic];
    //XAnimation add end
}
