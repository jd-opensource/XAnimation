...
- (instancetype)initWithContentsOfURL:(NSURL *)url {
  ...
  if (self) {
    ...
    if (laScene) {
      ...
      [self _callLoadCompletionIfNecessary:YES];
    } else {
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
        NSData *animationData = [NSData dataWithContentsOfURL:url];
        if (!animationData) {
            [self _callLoadCompletionIfNecessary:NO];
          return;
        }
        NSError *error;
...
- (void)_setupWithSceneModel:(LOTComposition *)model {
  ...
  _sceneModel = model;
  _compContainer = [[LOTCompositionContainer alloc] initWithModel:nil inLayerGroup:nil withLayerGroup:_sceneModel.layerGroup withAssestGroup:_sceneModel.assetGroup]; //@wxy 初始化混合层
...
- (void)_callLoadCompletionIfNecessary:(BOOL)loadFinished {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.loadCompletionBlock) {
            self.loadCompletionBlock(loadFinished);
        }
    });
}
...
- (void)playWithCompletion:(LOTAnimationCompletionBlock)completion {
  ...
  [self playFromFrame:_sceneModel.startFrame toFrame:_sceneModel.endFrame withCompletion:completion]; //@wxy 1.设置动画
}
...
- (void)playFromFrame:(nonnull NSNumber *)fromStartFrame
  ...
  if (!self.window) {
    ...
    animation.fillMode = kCAFillModeBoth;
      animation.repeatCount = _loopAnimationCount > 0 ? _loopAnimationCount : (_loopAnimation ? HUGE_VALF : 1) ;
